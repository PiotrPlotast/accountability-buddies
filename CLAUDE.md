...
**Notification preferences are per account; the accent and haptics are per device.** `hooks/useNotificationPrefs.ts` reads and writes `notification_prefs` through `queryKeys.notificationPrefs(userId)`, hand-rolled on the same shape as `useUpdateProfile` — cancel, snapshot, patch, roll back with an `Alert` (and the `error()` haptic), invalidate on settle. It **updates and never upserts**: the row is created by a trigger on `auth.users` and there is no client INSERT policy, so an upsert is rejected rather than helpful. `prefs` is `null` until the row is read — "not read yet", never "everything off", so the switches don't flash the wrong answer.

**`usePushPermission` is what keeps the settings screen honest.** It returns `null` while the OS answer is unread (the same distinction `useNameGate` draws between `isResolved` and `needsName`; rendering `null` as "blocked" would warn people about a permission they have), re-reads on `AppState` `"active"` because the only route back for a denied user is iOS Settings, and on the **transition** into granted calls `registerAndStorePushToken` — registration itself is keyed on the user id and will not run again that launch, so without it the screen says "allowed" while nothing can reach the phone until the next cold start. Returning to the foreground is not consent; only the transition is.

**The switches stay usable while the OS is blocking.** `notification_prefs` governs every device the person signs in on, so disabling the switches on a phone whose permission is denied would silently change what their other phone receives. The screen shows a permission row with `Linking.openSettings()` instead.

**One write of `notification_prefs.timezone`, from `usePushRegistration`.** It runs whatever the permission outcome and unconditionally once per launch, so the column follows the person when they travel — E5's reminder cron evaluates `now() at time zone timezone` for every row, and a row left at the `UTC` default fires someone's reminders at the wrong hour.
...
`hapticsEnabled` is a per-device preference alongside the accent, and the context copy exists only so the settings row in `Profile.tsx` has something to render — the copy that call sites actually obey lives in `lib/haptics.ts`, which the provider syncs on hydration and on every toggle. That switch now lives on `app/(protected)/notification-settings.tsx` beside the notification switches — one settings surface, not two — and `Profile.tsx` keeps only the gear that opens it.
